...
// ─── REPARAR ESTRUTURA DA CHAVE ──────────────────────────────────────────────
// Evita a tela branca que acontecia ao apagar uma fase do meio (ex: apagar a
// Semifinal mas deixar a Final) — depois disso as fases ficavam com um número
// de jogos incompatível entre si e o Bracket quebrava ao tentar desenhar.
// Esta função roda sempre que a chave é alterada (adicionar/remover fase ou
// jogo) e garante que cada fase tenha sempre metade dos jogos da anterior
// (arredondando pra cima), completando ou cortando o necessário.
function repairRounds(rounds) {
  try {
    if (!rounds || !rounds.length) return rounds;
    const r = rounds.map(rn => ({ ...rn, matches: [...(rn.matches||[])] }));
    for (let ri=1; ri<r.length; ri++) {
      const expected = Math.max(1, Math.ceil(r[ri-1].matches.length/2));
      const cur = r[ri].matches;
      if (cur.length > expected) {
        r[ri] = { ...r[ri], matches: cur.slice(0, expected) };
      } else if (cur.length < expected) {
        const extra = Array.from({ length: expected-cur.length }, () => window.mkMatch());
        r[ri] = { ...r[ri], matches: [...cur, ...extra] };
      }
    }
    return r;
  } catch(e) {
    console.error('repairRounds error:', e);
    return rounds;
  }
}
window.repairRounds = repairRounds;
...
// ─── REDE DE SEGURANÇA (evita tela branca em qualquer modalidade) ───────────
// Se qualquer erro de renderização acontecer dentro dela (chave desalinhada,
// dado corrompido vindo do Firebase, etc.), mostra um aviso amigável em vez
// de deixar o app inteiro travado em branco. Permite resetar só os
// resultados da modalidade afetada, sem perder o resto do site.
class ErrorBoundary extends React.Component {
  constructor(props){ super(props); this.state = { hasError:false }; }
  static getDerivedStateFromError(){ return { hasError:true }; }
  componentDidCatch(error, info){ console.error('Erro capturado pela ErrorBoundary:', error, info); }
  render(){
    if (this.state.hasError) {
      return (
        <div style={{minHeight:320,display:"flex",flexDirection:"column",alignItems:"center",
          justifyContent:"center",gap:14,padding:"40px 24px",textAlign:"center",
          fontFamily:"'Inter',sans-serif",background:"#0a0f00"}}>
          <div style={{fontSize:46}}>⚠️</div>
          <div style={{fontFamily:"'Bebas Neue',cursive",fontSize:22,letterSpacing:3,color:"#ef4444"}}>
            ALGO DEU ERRADO NESSA PÁGINA
          </div>
          <div style={{color:"rgba(255,255,255,.4)",fontSize:13,maxWidth:420,lineHeight:1.5}}>
            Provavelmente uma fase da chave ficou desalinhada. O resto do site continua
            funcionando normalmente — você pode tentar de novo ou resetar só os
            resultados desta modalidade (os nomes são mantidos).
          </div>
          <div style={{display:"flex",gap:8,marginTop:6}}>
            <button onClick={()=>this.setState({hasError:false})}
              style={{background:"transparent",border:"1px solid rgba(255,255,255,.2)",
                color:"rgba(255,255,255,.6)",borderRadius:6,padding:"9px 16px",
                fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>
              Tentar de novo
            </button>
            {this.props.onReset && (
              <button onClick={()=>{ this.props.onReset(); this.setState({hasError:false}); }}
                style={{background:"#fb923c",color:"#fff",border:"none",borderRadius:6,
                  padding:"9px 18px",fontWeight:800,fontSize:12,cursor:"pointer",
                  fontFamily:"'Inter',sans-serif"}}>
                🔄 Resetar resultados desta modalidade
              </button>
            )}
          </div>
        </div>
      ...
      <div className="body-wrap">
        ...
        <main className="main-scroll" style={{WebkitOverflowScrolling:"touch"}}>
          <div className="main-inner">
            ...
            {curMod&&(
              <ErrorBoundary key={"eb-"+page} onReset={()=>{
                const nd=window.resetarMod(data,curMod.id);
                setData(nd);
                if(window._fbSet)window._fbSet(nd);
                showToast('Resultados de '+curMod.name+' resetados ✓');
              }}>
                <ModalityPage key={page} mod={curMod} onChange={handleChange} canEdit={canEdit} isMobile={isMobile} isAdmin={isAdmin}
                  onResetMod={()=>{
                    const nd=window.resetarMod(data,curMod.id);
                    setData(nd);
                    if(window._fbSet)window._fbSet(nd);
