...
function getMatchY(ri, mi, rounds) {
  // Blindagem: fase vazia/ausente nunca deve quebrar o render (tela branca).
  const maxM = Math.max(1, ...rounds.map(r => (r?.matches?.length)||0));
  const n    = Math.max(1, rounds[ri]?.matches?.length||0);
  const step = (maxM * (CARD_H + V_GAP)) / n;
  return mi * step + (step - CARD_H) / 2;
}
...
function MatchCard({ match, gc, canEdit, onWin, onEdit }) {
  ...
  const rowStyle = (n, pi) => ({
    ...
    fontWeight: match.winner === n ? 800 : 400,
    fontSize: 12,
  });
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${isWon ? gc+"55" : "rgba(255,255,255,.1)"}`,
      borderRadius: 8, overflow: "hidden", position: "relative",
      opacity: isEmpty ? .3 : 1,
      boxShadow: isWon ? `0 0 14px ${gc}22` : "none",
      height: "100%",
    }}>
      {canEdit && (
        <button onClick={() => onEdit(match)} style={{
          position: "absolute", top: 3, right: 3,
          background: "rgba(0,0,0,.3)", border: "none", borderRadius: 3,
          color: "rgba(255,255,255,.5)", cursor: "pointer",
          fontSize: 10, zIndex: 1, padding: "1px 4px",
        }}>✏️</button>
      )}
      ...
      {soloCard ? (
        <div
          ...
          style={{
            ...
            color: match.winner === match.p1 ? gc : "#e2e8f0",
            fontWeight: match.winner === match.p1 ? 800 : 400,
            fontSize: 13,
          }}
        >
          {match.winner === match.p1 && (
            <div style={{ width:14, height:14, borderRadius:"50%", background:gc,
              display:"flex", alignItems:"center", justifyContent:"center",
              fontSize:8, color:"#0a0f00", fontWeight:900, flexShrink:0 }}>✓</div>
          )}
        ...
        [match.p1, match.p2].map((n, pi) => (
          <div key={pi}
            onClick={() => canEdit && n && n !== "BYE" && onWin(match.id, n)}
            style={rowStyle(n, pi)}
          >
            {match.winner === n && (
              <div style={{ width:14, height:14, borderRadius:"50%", background:gc,
...
// ─── BRACKET ─────────────────────────────────────────────────────────────────
function Bracket({ rounds: roundsRaw, gc, canEdit, onWin, onEdit, onAddMatch, onRename, onAddRound, onRemoveRound, onMoveRound }) {
  // Blindagem: qualquer fase sem "matches" (ex: apagada/desalinhada) vira uma
  // fase vazia em vez de quebrar a tela toda com tela branca.
  const rounds = (roundsRaw||[]).map(r => ({...r, matches: r?.matches||[]}));
  if (!rounds.length) return null;
  const champ      = rounds[rounds.length-1]?.matches?.[0]?.winner;
  const maxMatches = Math.max(1, ...rounds.map(r => r.matches.length));
  const totalH     = maxMatches * (CARD_H + V_GAP) + 60;
      ...
      <div style={{ position:"relative", width:totalW, height:totalH }}>
        ...
        {rounds.map((round, ri) => (
          <div key={round.id}>
            ...
            <div style={{ position:"absolute", left:ri*(CARD_W+H_GAP), top:0, width:CARD_W,
              ...
              {canEdit && ri > 0 && (
                <button onClick={() => onMoveRound(round.id, "left")}
                  style={{ background:"none", border:"none", color:`${gc}88`,
              ...
              {canEdit ? (
                <input value={round.name} onChange={e => onRename(round.id, e.target.value)}
                  style={{ background:"transparent", border:"none",
                    borderBottom:`1px solid ${gc}44`, color:gc,
                ...
                <span style={{ fontSize:9, fontWeight:800, letterSpacing:2,
                  ...
                  padding:"3px 10px", background:`${gc}14`,
              ...
              {canEdit && (
                <button onClick={() => onRemoveRound(round.id)}
                  style={{ background:"none", border:"none", color:"#ef4444",
                    cursor:"pointer", fontSize:11, padding:"0 2px" }}>🗑</button>
              )}
            </div>
            ...
            {round.matches.map((match, mi) => {
              ...
              const y = getMatchY(ri, mi, rounds) + 20;
              return (
            ...
            {canEdit && (
              <div style={{ position:"absolute", left:ri*(CARD_W+H_GAP),
                top: getMatchY(ri, round.matches.length, rounds) + 20 + CARD_H + 6 }}>
                <button onClick={() => onAddMatch(round.id)} style={{
                  background:"transparent", border:`1px dashed ${gc}44`,
                  borderRadius:6, padding:"4px 8px", color:`${gc}77`,
        ...
        {canEdit && (
          <div style={{ position:"absolute",
            left: rounds.length * (CARD_W + H_GAP) + 4,
            top: getMatchY(0, 0, rounds) + 20 }}>
            <button onClick={onAddRound} style={{
              background:"transparent",
              border:"1px dashed rgba(255,223,0,.28)",
              borderRadius:8, padding:"12px 9px",
              color:"rgba(255,223,0,.45)", fontSize:10,
        ...
        {champ && (
          <div style={{
            ...
            top: getMatchY(rounds.length-1, 0, rounds) + 20,
            ...
            <div style={{
              ...
              color:"#0a0f00", borderRadius:10, padding:"13px 18px",
              fontWeight:900, fontSize:14,
              boxShadow:"0 0 28px rgba(255,223,0,.45)",
...
function EditModal({ match, onSave, onRemove, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.8)", zIndex:400,
      ...
      <div style={{ background:"#0d1a05", border:"2px solid rgba(255,223,0,.25)",
        borderRadius:14, padding:28, width:"100%", maxWidth:340 }}>
        <div style={{ fontFamily:"'Bebas Neue',cursive", fontSize:20,
          letterSpacing:3, color:MGOLD, marginBottom:20 }}>✏️ EDITAR CONFRONTO</div>
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
          <button onClick={() => onRemove(match.id)} style={{ background:"transparent",
            border:"1px solid rgba(239,68,68,.3)", color:"#ef4444",
            borderRadius:6, padding:"8px 14px", fontSize:12, cursor:"pointer" }}>Remover</button>
          <button onClick={onClose} style={{ background:"transparent",
            border:"1px solid rgba(255,255,255,.2)", color:"rgba(255,255,255,.4)",
            borderRadius:6, padding:"8px 14px", fontSize:12, cursor:"pointer" }}>Cancelar</button>
          <button onClick={() => { onSave(match.id, p1||null, p2||null); onClose(); }}
            style={{ background:"linear-gradient(135deg,#009C3B,#006622)",
              color:"#fff", border:"none", borderRadius:6,
              padding:"8px 18px", fontWeight:700, fontSize:12, cursor:"pointer" }}>✓ Salvar</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.8)", zIndex:400,
      ...
      <div style={{ background:"#0d1a05", border:"2px solid rgba(255,223,0,.25)",
        ...
        <div style={{ fontFamily:"'Bebas Neue',cursive", fontSize:20,
          letterSpacing:3, color:MGOLD, marginBottom:16 }}>+ ADICIONAR JOGADORES</div>
        <div style={{ color:"rgba(255,255,255,.3)", fontSize:12, marginBottom:10 }}>
          Um nome por linha — serão adicionados à primeira fase</div>
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"flex-end", marginTop:14 }}>
          <button onClick={onClose} style={{ background:"transparent",
            border:"1px solid rgba(255,255,255,.2)", color:"rgba(255,255,255,.4)",
            borderRadius:6, padding:"8px 16px", fontSize:12, cursor:"pointer" }}>Cancelar</button>
          <button onClick={add} style={{ background:"linear-gradient(135deg,#009C3B,#006622)",
            color:"#fff", border:"none", borderRadius:6,
            padding:"8px 20px", fontWeight:700, fontSize:12, cursor:"pointer" }}>✓ Adicionar</button>
        </div>
      </div>
    </div>
  );
...
function gerarChave(players, randomize) {
  ...
  while (prev.length > 1) {
    ...
    const n = next.length;
    rounds.push({ id:window.uid(),
      name:n===1?"Final":n===2?"Semifinal":n===4?"Quartas de Final":"Fase",
      matches:next });
    prev = next;
  }
  return rounds;
...
function GeradorChave({ mod, onSalvar, onClose, defaultCat, isNivel }) {
  ...
  const curT = isNivel
    ? (NIVEL_TABS||[]).find(t => t.key===cat) || {gc:mod.accent,label:"Categoria",key:cat}
    : {gc:mod.accent, label:"⚡ Misto", key:"misto"};
  const gc = curT.gc;
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.85)", zIndex:400,
      ...
      <div style={{ background:"#0a0f00", border:"2px solid rgba(255,223,0,.2)",
        width:"100%", maxWidth:1100, display:"flex", flexDirection:"column",
        maxHeight:"100vh", overflow:"hidden" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12,
          padding:"14px 20px", borderBottom:"2px solid rgba(255,223,0,.15)", flexShrink:0 }}>
          <span style={{ fontSize:22 }}>{mod.emoji}</span>
          <div style={{ fontFamily:"'Bebas Neue',cursive", fontSize:20,
            letterSpacing:3, color:MGOLD }}>GERADOR DE CHAVES · {mod.name}</div>
          <button onClick={onClose} style={{ marginLeft:"auto", background:"none",
            border:"none", color:"rgba(255,255,255,.4)", fontSize:22, cursor:"pointer" }}>✕</button>
        </div>
        <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
          <div style={{ width:280, flexShrink:0, borderRight:"1px solid rgba(255,255,255,.07)",
            padding:20, overflowY:"auto", background:"rgba(255,255,255,.02)" }}>
            <div style={{ marginBottom:14 }}>
              <div style={{ color:"rgba(255,255,255,.35)", fontSize:10, letterSpacing:1,
                textTransform:"uppercase", marginBottom:6 }}>Categoria</div>
              <div style={{ display:"flex", borderRadius:7, overflow:"hidden",
                border:"2px solid rgba(255,255,255,.1)", flexWrap:"wrap" }}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <textarea value={raw} onChange={e => setRaw(e.target.value)} rows={12}
                ...
                style={{ width:"100%", background:"rgba(255,255,255,.05)",
                  border:`1px solid ${gc}33`, borderRadius:6,
                  padding:"9px 11px", color:"#e2e8f0", fontSize:12,
                  outline:"none", fontFamily:"'Inter',sans-serif",
                  resize:"vertical", boxSizing:"border-box", lineHeight:1.7 }}/>
            </div>
            <div style={{ marginBottom:16 }}>
              <label style={{ display:"flex", alignItems:"center", gap:10, cursor:"pointer",
                ...
                <div onClick={() => setRandomize(!randomize)} style={{
                  width:38, height:21, borderRadius:11,
                  background:randomize?"#009C3B":"rgba(255,255,255,.12)",
                  cursor:"pointer", position:"relative", transition:"background .2s", flexShrink:0 }}>
                  <div style={{ position:"absolute", top:2, left:randomize?17:2,
            ...
            {players.length >= 2 && (
              <div style={{ padding:"10px 12px", background:`${gc}12`,
                border:`1px solid ${gc}28`, borderRadius:6, marginBottom:14, fontSize:11 }}>
            ...
            <button onClick={gerar} disabled={players.length<2} style={{
              width:"100%",
              background:players.length>=2?`linear-gradient(135deg,${gc},${gc}bb)`:"rgba(255,255,255,.07)",
              color:players.length>=2?"#fff":"rgba(255,255,255,.2)",
            ...
            {rounds && (
              <>
                <button onClick={() => setRounds(gerarChave(players,randomize))} style={{
                  width:"100%", background:"transparent", border:`1px solid ${gc}44`,
                  color:gc, borderRadius:8, padding:"9px", fontWeight:700, fontSize:12,
                  cursor:"pointer", fontFamily:"'Inter',sans-serif", marginBottom:8 }}>
                  🎲 Novo sorteio
                </button>
                <button onClick={salvar} style={{
                  width:"100%", background:"linear-gradient(135deg,#FFDF00,#FFB800)",
                  color:"#0a0f00", border:"none", borderRadius:8, padding:"12px",
                  fontWeight:900, fontSize:13, cursor:"pointer",
                  fontFamily:"'Inter',sans-serif", boxShadow:"0 4px 18px rgba(255,223,0,.3)" }}>
                  💾 SALVAR NO SITE
                </button>
              </>
            )}
          ...
          <div style={{ flex:1, padding:24, overflowX:"auto", overflowY:"auto" }}>
            {!rounds ? (
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center",
                justifyContent:"center", height:"100%", gap:14, opacity:.35 }}>
                <div style={{ fontSize:56 }}>🏆</div>
                <div style={{ fontFamily:"'Bebas Neue',cursive", fontSize:22,
                  letterSpacing:4, color:MGOLD }}>CONFIGURE E GERE A CHAVE</div>
              </div>
            ) : (
              <div>
                <div style={{ marginBottom:20 }}>
                  <div style={{ fontFamily:"'Bebas Neue',cursive", fontSize:24,
                    color:MGOLD, letterSpacing:4 }}>{mod.name}</div>
                  <div style={{ display:"flex", gap:8, marginTop:6, alignItems:"center" }}>
                    <div style={{ background:gc, borderRadius:4, padding:"3px 10px",
                      fontSize:11, fontWeight:800, color:"#fff" }}>{curT.label}</div>
                    <div style={{ color:"rgba(255,255,255,.3)", fontSize:11 }}>
                      {players.length} atletas · {rounds.length} fases</div>
                  </div>
                </div>
                <Bracket rounds={rounds} gc={gc} canEdit={false}
                  onWin={()=>{}} onEdit={()=>{}} onAddMatch={()=>{}}
                  onRename={()=>{}} onAddRound={()=>{}}
                  onRemoveRound={()=>{}} onMoveRound={()=>{}}/>
              </div>
            )}
          </div>
        </div>
